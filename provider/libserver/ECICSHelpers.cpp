    ...
    struct restrictTable *restrict, std::set<SOURCEKEY> **matches_p)
{
	...
	ec_log(EC_LOGLEVEL_ICS, "MatchRestrictions: matching %lu rows", db_rows.size());
	...
	ec_log(EC_LOGLEVEL_ICS, "MatchRestrictions: %lu match(es) out of %d rows (%d properties)", matches->size(), lpRowSet->__size, lpPropTags->__size);
