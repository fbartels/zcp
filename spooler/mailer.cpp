	...
	if (ulRows == 0 || (permanentFailedRecipients.empty() && temporaryFailedRecipients.empty())) {
		...
		if (!temporaryFailedRecipients.empty()) {
			...
			for(size_t i=0; i<temporaryFailedRecipients.size(); i++) {
				const sFailedRecip *const cur = &temporaryFailedRecipients.at(i);

				newbody.append(L"\t");
				newbody.append(cur -> strRecipName.c_str());
				newbody.append(L" <");
				newbody.append(converter.convert_to<WCHAR *>(cur -> strRecipEmail));
				newbody.append(L">\n");
			}
		}
		...
		if (!permanentFailedRecipients.empty()) {
			...
			for(size_t i=0; i<permanentFailedRecipients.size(); i++) {
				const sFailedRecip *const cur = &permanentFailedRecipients.at(i);
		...
		if ((hr = MAPIAllocateBuffer(CbNewADRLIST(temporaryFailedRecipients.size()), (void**)&lpMods)) != hrSuccess)
			goto exit;

		lpMods->cEntries = 0;

		for(size_t j = 0; j < temporaryFailedRecipients.size(); j++) {
			const sFailedRecip *const cur = &temporaryFailedRecipients.at(j);
			...
			lpMods->aEntries[cEntries].rgPropVals[ulPropModsPos++].Value.lpszA = (char *)cur -> strRecipEmail.c_str();
			...
			if (!cur -> strRecipName.empty())
				lpMods->aEntries[cEntries].rgPropVals[ulPropModsPos++].Value.lpszW = (WCHAR*)cur -> strRecipName.c_str();
			else
				lpMods->aEntries[cEntries].rgPropVals[ulPropModsPos++].Value.lpszW = converter.convert_to<WCHAR *>(cur -> strRecipEmail);

			lpMods->aEntries[cEntries].rgPropVals[ulPropModsPos].ulPropTag = PR_REPORT_TEXT_A;
			lpMods->aEntries[cEntries].rgPropVals[ulPropModsPos++].Value.lpszA = (char *)cur -> strSMTPResponse.c_str();
